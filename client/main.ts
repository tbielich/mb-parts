import { initPartsAdvisorChat } from './chatbot.js';
...
const liveSyncCacheKey = 'mb-parts.live-sync-cache.v1';
const liveSyncCacheMaxAgeMs = 24 * 60 * 60 * 1000;
...
function isUnavailableApiResponse(response: Response): boolean {
  return response.status === 404;
}

function markLiveApiUnavailable(): void {
  liveApiAvailable = false;
}

function markLiveApiAvailable(): void {
  liveApiAvailable = true;
}

function readCachedLiveSync(): CachedCatalogState | null {
  try {
    const raw = window.localStorage.getItem(liveSyncCacheKey);
    if (!raw) {
      return null;
    }

    const parsed = JSON.parse(raw) as CachedCatalogState;
    if (parsed.version !== 1 || !Array.isArray(parsed.items) || typeof parsed.cachedAt !== 'string') {
      window.localStorage.removeItem(liveSyncCacheKey);
      return null;
    }

    const cachedAt = Date.parse(parsed.cachedAt);
    if (!Number.isFinite(cachedAt) || Date.now() - cachedAt > liveSyncCacheMaxAgeMs) {
      window.localStorage.removeItem(liveSyncCacheKey);
      return null;
    }
...
function writeCachedLiveSync(items: PartItem[]): void {
  try {
    const payload: CachedCatalogState = {
      version: 1,
      cachedAt: new Date().toISOString(),
      items,
    };
    window.localStorage.setItem(liveSyncCacheKey, JSON.stringify(payload));
  } catch {
...
async function lazyEnrichVisibleItems(items: PartItem[]): Promise<void> {
  ...
  try {
    ...
    if (isUnavailableApiResponse(response)) {
      markLiveApiUnavailable();
      return;
    }
    if (!response.ok) {
...
async function syncAndReload(): Promise<void> {
  ...
  try {
    ...
    if (isUnavailableApiResponse(baseResponse)) {
      markLiveApiUnavailable();
    ...
    for (let i = 0; i < allPartNumbers.length; i += inStockSyncBatchSize) {
      ...
      if (isUnavailableApiResponse(response)) {
        markLiveApiUnavailable();
        setStatus('Detail-Sync lokal nicht verfügbar, verwende vorhandene Katalogdaten.');
        renderCurrentPage({ triggerLazy: false });
        return;
      }
      if (!response.ok) {
    ...
    writeCachedLiveSync(initialChunkItems);
    setStatus(
...
inStockOnlyCheckbox.addEventListener('change', async () => {
  ...
  if (inStockOnly) {
    setInStockSyncButtonState(true);
    try {
      const validatedCount = await validateUnknownAvailability();
      renderCurrentPage();
      const visibleCount = getVisibleItems().length;
      const summary = getAvailabilitySummary(allItems);
      if (liveApiAvailable) {
        setStatus(
          `Fertig: ${visibleCount}/${allItems.length} (lieferbar:${summary.inStock} ausverkauft:${summary.outOfStock} unbekannt:${summary.unknown}, Filter blendet nur ausverkauft aus${
            validatedCount > 0 ? `, geprüft ${validatedCount}` : ''
          })`,
        );
      } else {
        setStatus(
          `Fertig: ${visibleCount}/${allItems.length} (lieferbar:${summary.inStock} ausverkauft:${summary.outOfStock} unbekannt:${summary.unknown}, lokaler Modus ohne Live-Verfügbarkeitsprüfung)`,
        );
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unbekannter Fehler';
...
const cachedLiveSync = readCachedLiveSync();
if (cachedLiveSync) {
  initialChunkItems = cachedLiveSync.items;
  applyResultItems(initialChunkItems);
  const visibleCount = getVisibleItems().length;
  setStatus(
    `Cache geladen: ${visibleCount}/${allItems.length} (${allowedPartPrefixes.join('|')}), Stand ${formatCachedAt(cachedLiveSync.cachedAt)}`,
  );
} else {
